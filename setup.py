...
if __name__ == "__main__":
    ...
    for dot_file in DOT_FILES:
        if (input(f"Install {dot_file}? (Y/n): ").casefold() or "y") == "y":
            ...
            try:
                subprocess.run(
                    ["mv", str(local_dot_file), str(github_dot_file)], check=True
                )
                print(
                    f"Moved {local_dot_file} to {github_dot_file}. Check git diff to see the changes."
                )
            ...
            subprocess.run(
                ["ln", "-s", str(github_dot_file), str(local_dot_file)], check=False
            )
    ...
    if (install_homebrew == "y") and (
        input("Install the software in the brewfile? (Y/n): ").casefold() or "y"
    ) == "y":
        # install software in the brewfile
        subprocess.run(
            ["brew", "bundle", "--file", str(Path.home() / ".dotfiles" / "Brewfile")],
            check=False,
        )
    ...
    if (
        input(
            "Your .dotfiles is kept at ~/.dotfiles. Do you want to create a symlink to it? (Y/n): "
        ).casefold()
        or "y"
    ) == "y":
        sym_dir_input = input(
            "Enter the absolute path to the directory you want to create the symlink in: "
        )
        ...
        # Create symlink
        subprocess.run(
            ["ln", "-s", str(dotfiles_path), str(symlink_target)], check=False
        )
